...
trash.forEach((el) => {
  el.addEventListener("click", (e) => {
    ...
    popupContainer.appendChild(
      new ConfirmAlert({
        message: {
          confirmText: "Are you sure you want to delete this prime ?",
          confirmTitle: "Delete PrimeType",
        },
        parent: popupContainer,
        onConfirm: () => {
          const url = `/api/primes/${id}`;
          const options = {
            method: "DELETE",
            headers: {
                  ...
                  new Alert(
                    {
                      type: "success",
                      msg_title: "SUCCES",
                      msg_text: "PrimeType deleted successfully",
                    },
                    alertContainer
                  ).render()
                );
...
edit.forEach((el) => {
  el.addEventListener("click", function (e) {
    const id = e.target.dataset.id;
    const url = `/get_typeprime_form?id=${id}`;
    const options = {
      method: "GET",
      headers: {
        "X-CSRFToken": csrfToken,
